...
Chart.prototype.labelAxes = function() {
    ...
    if (this.xlabel) {
        ...
        charm.right(this.width/2 - String(this.xmin).length - this.xlabel.length/2);
        charm.write(this.xlabel);
        charm.right(this.width/2 - this.xlabel.length/2);
        var xmaxstr = this.direction === 'x' ? String(this.maxBound || this.max_size) : String(this.xmax);
        charm.write(xmaxstr);
        charm.pop();
...
Chart.prototype.drawBars = function() {
    ...
    if (this.direction === 'x') {
        this.scale = this.width/(this.maxBound || this.max_size);
    } else {
        this.scale = this.height/(this.maxBound || this.max_size);
    }
